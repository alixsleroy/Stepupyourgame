...
- g4(x,dtbounds): the adaptive function is based on the double well potential bounded between two values 
- e_m_ada4(y0,s,b1,dt,dtbounds): a function that computes the next value of the increment using euler maruyama on the transformed SDE
- run_num_ada4(Ntot,dt,s,dtbounds): a function that goes through all the iterations 
- IDW_nsample_ada4(n_samples,T,dt,tau,dtbounds): a function that runs the EM increments for a number of sample
"""
...
def e_m_ada(y0,s,b1,dt,dtbounds):
    """
    ...
    y1=y0-getminusdU(y0)*newdt+nablag*dt+s*b1*np.sqrt(gy)
...
def run_num_ada(T,dt,s,dtbounds):
    """
    ...
    # for jj in range(Ntot): # Run until T= Tsec
        ...
        yt = e_m_ada(y0,s,b1,dt,dtbounds)
        y1=yt[0]
        newdt=yt[1]
        y0=y1 
...
@njit(parallel=True)
def nsample_ada(n_samples,T,dt,dtbounds): # Function is compiled and runs in machine code
    ...
    for i in range(n_samples):
        yf_t =run_num_ada(T,dt,s,dtbounds)
        yt_list[i,::]=yf_t
    return yt_list
