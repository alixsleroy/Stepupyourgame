...
The adaptive function is the constant slope determined by dtbounds[0], dtbounds[1] and R.
...
- g2(x,dtbounds): the adaptive function used here
- e_m_ada2(y0,s,b1,dt,dtbounds): a function that computes the next value of the increment using euler maruyama on the transformed SDE
- run_num_ada2(Ntot,dt,s,dtbounds): a function that goes through all the iterations 
- IDW_nsample_ada2(n_samples,T,dt,tau,dtbounds): a function that runs the EM increments for a number of sample
"""
...
def e_m_ada2(y0,s,b1,dt,dtbounds):
    """
    ...
    gy=re[0]
    nablag=re[1]
    y1=y0-gy*minusdU(y0)*dt+nablag*dt+np.sqrt(gy)*s*b1
    return y1    
...
def run_num_ada2(Ntot,dt,s,dtbounds):
    """
    ...
    for jj in range(Ntot): # Run until T= Tsec
        ...
        y1 = e_m_ada2(y0,s,b1,dt,dtbounds)
        y0=y1 
    return (y0)
...
@njit(parallel=True)
def IDW_nsample_ada2(n_samples,T,dt,tau,dtbounds): # Function is compiled and runs in machine code
    """
    Input
    -------
    n_samples: int
    ...
    for i in range(n_samples):
        yf =run_num_ada2(Ntot,dt,s,dtbounds)
        y_final.append(yf)
    y_final=np.array(y_final)
    return y_final

ytest= IDW_nsample_ada2(10**2,3,0.1,10,np.array([0.001,0.1])) # compile the function
