...
#define T               200          // Time to integrate to
...
// //#define PATH "./validate/spring_v1"

// vector<double> dtlist ={0.15  , 0.1744, 0.1989, 0.2233, 0.2478, 0.2722, 0.2967, 0.3211,
//        0.3456, 0.37  };
// #define r     0.05
// #define d     5.
// #define m1    .3
...
vector<double> one_step_tr(double dt, double numruns, int i)
{
    ...
    string file_name=path+"/vec_tr_springv1_q"+list_para+".txt";
    file.open(file_name,ios_base::out);
    ostream_iterator<double> out_itr(file, "\n");
    copy(vec_q.begin(), vec_q.end(), out_itr);
    file.close();

    file_name=path+"/vec_tr_springv1_p"+list_para+".txt";
    file.open(file_name,ios_base::out);
    copy(vec_p.begin(), vec_p.end(), out_itr);
    file.close();

    file_name=path+"/vec_tr_springv1_g"+list_para+".txt";
    file.open(file_name,ios_base::out);
...
file << fixed << setprecision(16) << endl;
string list_para="i="+to_string(i); 
string file_name=path+"/vec_springv1_q"+list_para+".txt";
file.open(file_name,ios_base::out);
ostream_iterator<double> out_itr(file, "\n");
copy(vec_q.begin(), vec_q.end(), out_itr);
file.close();

file_name=path+"/vec_springv1_p"+list_para+".txt";
file.open(file_name,ios_base::out);
copy(vec_p.begin(), vec_p.end(), out_itr);
file.close();
...
int main(void) {    
    ...
    for(int i = 0; i < dtlist.size(); i++){ // run the loop for ns samples
        ...
        moments_tr_4[i]=moments_di[3]; //in this we have stocked the average value of g
        
        // * For equivalent comparison, I have used 
        // double dti_g=dti*moments_di[3];
        // // cout<<"g\n";
        // // cout<<moments_di[3];
        // // cout<<"\ndt\n";
        // // cout<<dti;
