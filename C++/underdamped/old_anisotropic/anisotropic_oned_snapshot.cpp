...
// #define printskip       10
#define numruns        80000          // total number of trajectories
#define numsam         10 //10000        // total number of trajectories
#define nsnapshot      5
double printskip=int(numruns/nsnapshot);

/////////////////////////////////
// Asymetric double well 1 d definition //
/////////////////////////////////
#define a   2
#define c   1
...
{
    double xc=(x-c);
    double res=((x+a)*(x+a)-0.1)*(xc*xc*xc*xc);
    return res;
}
...
double Up(double x)
{
    double xc=(x-c);
    double xa=x+a;
    double res=2*xc*xc*xc*(xc*xa+2*xa*xa-0.2);
    return res;
}
...
int one_step(void)
{
    ...
    vector<vector<double>> vec_q(nsnapshot,q_list);
    vector<vector<double>> vec_p(nsnapshot,q_list);
    ...
    for(int nsps = 0; nsps<nsnapshot; nsps++){
        file_name=path+"/snapshot/vec_noada"+to_string(nsps)+".txt";
...
int one_step_tr(void)
{
    ...
    vector<vector<double>> vec_q(nsnapshot,q_list);
    vector<vector<double>> vec_p(nsnapshot,q_list);
    vector<vector<double>> vec_g(nsnapshot,q_list);
    ...
    string path="/home/s2133976/OneDrive/ExtendedProject/Code/Stepupyourgame/Stepupyourgame/data/C/underdamped/1d";
    for(int nsps = 0; nsps<nsnapshot; nsps++){
        file_name=path+"/snapshot/vec_tr"+to_string(nsps)+".txt";
        file.open(file_name,ios_base::out);
        ostream_iterator<double> out_itr(file, "\n");
        file<<"q\n";
        copy(vec_q[nsps].begin(), vec_q[nsps].end(), out_itr);
