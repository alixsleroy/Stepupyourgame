...
// Anisotropic  //
////////////////// 
vector<double> dtlist = {0.01 , 0.014, 0.019, 0.023, 0.028, 0.032, 0.037, 0.041, 0.046,
       0.05};
// {exp(-5.3), exp(-5.), exp(-4.79), exp(-4.53), exp(-4.27), exp(-4.02), exp(-3.76), exp(-3.51), exp(-3.25), exp(-3.)};
...
// vector<double> dtlist = {0.02      , 0.02241379, 0.02482759, 0.02724138, 0.02965517,
//        0.03206897, 0.03448276, 0.03689655, 0.03931034, 0.04172414,
//        0.04413793, 0.04655172, 0.04896552, 0.05137931, 0.0537931 ,
//        0.0562069 , 0.05862069, 0.06103448, 0.06344828, 0.06586207,
//        0.06827586, 0.07068966, 0.07310345, 0.07551724, 0.07793103,
//        0.08034483, 0.08275862, 0.08517241, 0.08758621, 0.09      };
...
vector<double> nt_steps_no_ada(double ds, double numruns, int i)
{
    ...
    for(int ns = 0; ns <= numsam; ns++){ // run the loop for ns samples
        ...
        // compute the moments
        if (isnan(x1)==true or isnan(y1)==true){x1=9999999; y1=9999999;moments[3]+=1;}
        moments[0]+=x1;
        moments[1]+=y1;
        moments[2]+=x1*y1;
